...
class TSnackbarUitils {
    companion object {
        fun toSuccess(activity: Activity, message: String): TSnackbar {
            return TSnackbar.make(activity.getRootView(), message, TSnackbar.LENGTH_LONG, TSnackbar.APPEAR_FROM_TOP_TO_DOWN).setPromptThemBackground(Prompt.SUCCESS)
                    .setBackgroundColor(activity.getColorSource(R.color.primary_dark))
        }

        fun toFail(activity: Activity, message: String): TSnackbar {
            return TSnackbar.make(activity.getRootView(), message, TSnackbar.LENGTH_LONG, TSnackbar.APPEAR_FROM_TOP_TO_DOWN).setPromptThemBackground(Prompt.ERROR)
