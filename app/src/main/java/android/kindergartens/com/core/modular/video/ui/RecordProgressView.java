...
//    private float mTotalWidth;
    ...
    private void init(){
        ...
        mBackgroundColor = getResources().getColor(android.kindergartens.com.R.color.record_progress_bg);
        mRecordColor = getResources().getColor(android.kindergartens.com.R.color.record_progress);
        mPendingColor = getResources().getColor(android.kindergartens.com.R.color.record_progress_pending);
        mSpaceColor = getResources().getColor(android.kindergartens.com.R.color.white);
        ...
        for(ClipInfo clipInfo : mClipInfoList){
            ...
            switch (clipInfo.clipType){
                case ClipInfo.CLIP_TYPE_SPACE:
                    canvas.drawRect(totalWidth - getResources().getDimension(android.kindergartens.com.R.dimen.ugc_progress_divider), 0f, newWidth, getHeight(), mSpacePaint);
                    break;
...
//                    mPaint.setShader(progressGradient);
        ...
        if(lastTotalProgress + mCurClipInfo.progress < mMinDuration){
            canvas.drawRect(mMinDuration / (float)mMaxDuration * getWidth(), 0f,
                    mMinDuration / (float)mMaxDuration * getWidth() + getResources().getDimension(android.kindergartens.com.R.dimen.ugc_progress_min_pos), getHeight(), mSpacePaint);
        }
        if(isCursorShow || isInProgress){
            canvas.drawRect(totalWidth, 0f, totalWidth + getResources().getDimension(android.kindergartens.com.R.dimen.ugc_progress_cursor), getHeight(), mSpacePaint);
        }
    }
